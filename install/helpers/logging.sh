start_install_log() {
  ...
  echo "=== Omakube Installation Started: $OMAKUB_START_TIME ===" >>"$OMAKUB_INSTALL_LOG_FILE"
}

stop_install_log() {
  if [[ -n ${OMAKUB_INSTALL_LOG_FILE:-} ]]; then
    OMAKUB_END_TIME=$(date '+%Y-%m-%d %H:%M:%S')
    echo "=== Omakube Installation Completed: $OMAKUB_END_TIME ===" >>"$OMAKUB_INSTALL_LOG_FILE"
    ...
    if [ -n "$OMAKUB_START_TIME" ]; then
      ...
      echo "Omakube:     ${OMAKUB_MINS}m ${OMAKUB_SECS}s" >>"$OMAKUB_INSTALL_LOG_FILE"
